...
readonly DEFAULT_TFTPD_PORT=69
...
readonly PATH_BIN_TFTPD="/usr/sbin/in.tftpd"
...
boot_main_tftpd() {
  ...
  local -r port="${DEFAULT_TFTPD_PORT}"
  local args=('-L' '--user' 'tftp' '-a' "0.0.0.0:${port}" '--secure' '-v' '/srv/nfs/exports/beacon_root/boot')
  $PATH_BIN_NFSD
  boot_helper_start_non_daemon "starting tftpd on port $port" $PATH_BIN_TFTPD "${args[@]}"
}
